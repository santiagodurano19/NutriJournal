...
const s: { [key: string]: React.CSSProperties } = {
  ...
  errorBox: { backgroundColor: 'rgba(251, 113, 133, 0.1)', border: `1px solid rgba(251, 113, 133, 0.2)`, borderRadius: '16px', padding: '12px 16px', color: theme.colors.error, fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }
};
...
export const JournalPortal: React.FC<JournalPortalProps> = ({ onJournalSelect }) => {
  ...
  const renderAuth = () => (
    <div style={s.card}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        <div style={{ width: '72px', height: '72px', backgroundColor: theme.colors.primary, borderRadius: '24px', margin: '0 auto 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <ShieldCheck color="white" size={36} />
        </div>
        <h1 style={{ fontSize: '30px', fontWeight: '900', margin: '0 0 10px' }}>JOURNAL</h1>
        <p style={{ color: theme.colors.textMuted }}>{authMode === 'register' ? 'Crea tu acceso unificado' : 'Bienvenido de nuevo'}</p>
      </div>
      {errorMessage && <div style={s.errorBox}><AlertCircle size={16} /> {errorMessage}</div>}
  ...
  const renderHub = () => (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <h2 style={{ fontSize: '48px', fontWeight: '900', marginBottom: '64px' }}>JOURNAL HUB</h2>
      <div style={s.hubGrid}>
        <div onClick={() => onJournalSelect('nutri')} style={{ ...s.hubCard, borderColor: `${theme.colors.accent}44` }}>
          <div style={{ width: '64px', height: '64px', backgroundColor: `${theme.colors.accent}15`, borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '32px' }}>
            <Sparkles color={theme.colors.accent} size={30} />
          </div>
          <h3 style={{ fontSize: '24px', fontWeight: '800' }}>Nutri Journal</h3>
          <p style={{ color: theme.colors.textMuted }}>Optimización biológica y seguimiento nutricional asistido por IA.</p>
